...
class MF_adaboost(BaseRecommender):
    ...
    def train_model(self, dataset, evaluator, early_stop, logger, config):
        ...
        test_score_output['MDG@20'] = avg_mdg  # This ensures it shows up in Main.py!
    ...
    def calculate_item_scores(self, k=20):
        """
        Calculates MDG@K for ITEMS using Design 2 (CFBoost) logic.
        """
        self.eval()
        print("Calculating Item MDG scores for CFBoost... (This takes a moment)")
        ...
        # Calculate denominator (popularity in test set)
        ...
        with torch.no_grad():
            ...
            for start in range(0, num_users, batch_size):
                ...
                # === CFBoost Design 2 Prediction Logic for Batch ===
                batch_rec = None
                for i, stump in enumerate(self.stumps):
                    # Load saved vectors (slicing users for batch)
                    u_vec = np.load(self.u_file + '/' + str(i) + '_u.npy', allow_pickle=True)[batch_users]
                    i_vec = np.load(self.i_file + '/' + str(i) + '_i.npy', allow_pickle=True)
                    
                    # Alpha Calculation
                    err = torch.matmul(torch.Tensor(u_vec).float().to(self.device),
                                       torch.Tensor(i_vec.T).float().to(self.device))
                    stump_weight = torch.log((self.beta1) / (self.beta2 + err))
                    alpha = F.softmax(stump_weight / self.tau)
...
class MF(BaseRecommender):
    ...
    def make_records(self):  # record all the results' details into files
        ...
        if not os.path.exists(similarity_dir):
            os.mkdir(similarity_dir)
        similarity_file = os.path.join(similarity_dir, 'PC_CF_saves')
        if not os.path.exists(similarity_file):
            os.mkdir(similarity_file)
        with open(os.path.join(similarity_file,'P_CF.npy'), 'wb') as f:
            np.save(f, P)
        with open(os.path.join(similarity_file,'Q_CF.npy'), 'wb') as f:
            np.save(f, Q)
        # return P, Q
